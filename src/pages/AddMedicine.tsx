...
const AddMedicine = () => {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    manufacturer: "",
    batchNumber: "",
    expiryDate: new Date(new Date().setMonth(new Date().getMonth() + 12)), // Default to 1 year from now
    price: 0,
    stock: 0,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_URL}/stores/${storeId}/medicines`, formattedData);
      ...
      toast({
        title: "Error",
        description: error.response?.data?.detail || "Failed to add medicine. Please try again.",
        variant: "destructive",
      });
    } finally {
      setSubmitting(false);
  ...
  return (
    ...
    >
      <div className="flex items-center mb-8">
        ...
        <h1 className="text-2xl md:text-3xl font-bold">Add New Medicine</h1>
      </div>
